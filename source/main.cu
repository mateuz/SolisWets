...
int main(int argc, char * argv[]){
  ...
  uint f_id      = 2;
  uint n_threads = 32;
  uint n_blocks  = iDivUp(n_dim, n_threads);

  float x_min = -100.0;
  float x_max = +100.0;
  float delta = 0.4 * (x_max - x_min);

  SolisWets *sw = new SolisWets(n_success, n_fails, n_dim, n_threads, n_blocks, f_id, x_min, x_max, delta);
  ...
  sw->optimize(1000000, thrust::raw_pointer_cast(sol.data()));
